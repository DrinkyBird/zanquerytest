...
const SQF2_ALL = ( SQF2_PWAD_HASHES|SQF2_COUNTRY|SQF2_GAMEMODE_NAME|SQF2_GAMEMODE_SHORTNAME|0 );
...
function onMessage(msg, rinfo) {
    ...
    if (response == SERVER_LAUNCHER_CHALLENGE) {
        ...
        let flags = readLong();
        handleFields(0, flags);
        isEnd = true;
    } else if (response == SERVER_LAUNCHER_SEGMENTED_CHALLENGE) {
        const segmentNumber = readByte();
        const size = readLong();
        const bits = readLong();
        const flagsetNumber = readByte();
        
        isEnd = (segmentNumber & (1 << 7));
        
        console.log("Segment %d%s, flagset %d, bits %s, size %d", segmentNumber & ~(1<<7), isEnd ? " (end)" : "", flagsetNumber, bits.toString(16), size);
        ...
        try {
            handleFields(flagsetNumber, bits);
        } catch (ex) { }
    }
...
function handleFields(fieldsetNum, flags) {
    if (fieldsetNum == 0) {
        if (flags & SQF_NAME) {
            let name = readString();
    ...
    if (flags & SQF_EXTENDED_INFO) {
        const flags2 = readLong();
        console.log("Ext flags: %d", flags2);
        try {
            handleFields(fieldsetNum + 1, flags2);
        } catch (ex) { throw ex;}
    }
}
...
function main(args) {
    ...
    off = buf.writeInt32LE(wantedFlags2, off);
